...
,max(case when ef.ms_drg_icd10 in ('870','871','872') then 1 else 0 end ) as sepsis_icd_ind_csa -- FY23 change
...
--	, case when (pds.updateid = 'Incarcerated') then 0 else 1 end as crnr_lnsg_prsnr_excl_ind
	...
	,tqa.COPD_ind
	,tqa.sepsis_icd_ind
	,tqa.sepsis_icd_ind_csa -- FY23 change
...
ef.sepsis_icd_ind_csa, --FY23 changes
ef.non_covid_case_ind,
ef.expc_mrtly_outc_case_cnt, 
1 as join_key
FROM stg_encntr_qs_anl_fct_vw ef
...
--CSA Sepsis Mortality Index
	...
	(
		select 
			...
			count(distinct encntr_num) as total_encntrs,
			(SUM( case when tm.mrtly_outc_case_w_excl=0 then null 
			 when tm.hspc_pyr_excl=1 and tm.sepsis_icd_ind_csa=1  then mrtly_cnt_w_excl
			 else null
			end)) as mort_obs_agg_cases,
			 
			(SUM( case when tm.mrtly_outc_case_w_excl=0 then null
			 when tm.sepsis_icd_ind_csa=1 and tm.hspc_pyr_excl =1 then tm.csa_expc_mrtly_cnt
			 else null end
			)) as mort_expc_agg_cases, 

			count(distinct(case when tm.sepsis_icd_ind_csa=1 and tm.hspc_pyr_excl =1 and tm.expc_mrtly_outc_case_cnt =1 then tm.encntr_num else null end)) as mrtly_outc_cases
